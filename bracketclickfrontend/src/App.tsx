...
function App() {
  ...
  try {
    const finalEmail = (savedEmail || email).trim();

    if (!finalEmail || selectedFrame === null) {
      console.warn("Missing email or frame selection");
      return;
    }

    const res = await fetch("http://127.0.0.1:5000/api/selection", {
      method: "POST",
    ...
    console.log("selection response:", res.status, data);
  ...
  if (selectedFrame === null) {
    console.warn("Select frame first");
    return;
  }
